...
phase: 3
inputs: [companyfacts.json, statements_required_{date}.json]
outputs: [company-facts-keys.json (XBRL tag inventory), csv/{tag}.csv (one per relevant tag), {form}_{period_end}_raw.json (one per required filing)]
depends_on: [pull-raw-data, identify-required-statements]
feeds_into: [last-twelve-months, last-10-k]
*/}
